...
export class TrustScoreEngine {
  ...
  private async fetchTxActivity(address: string): Promise<{ count: number; frequency30d: number }> {
    try {
      // Etherscan-compatible API — transaction history
      const headers = this.buildOkxHeaders("GET", `/api/v5/explorer/address/transaction-list?address=${address}&chainShortName=ETH_SEPOLIA&limit=50`);
      const res = await axios.get(
        `https://www.oklink.com/api/v5/explorer/address/transaction-list?address=${address}&chainShortName=ETH_SEPOLIA&limit=50`,
  ...
  private async fetchDexActivity(address: string): Promise<{ txCount: number }> {
    try {
      // Etherscan-compatible API — DeFi transaction history
      const headers = this.buildOkxHeaders("GET", `/api/v5/explorer/address/token-transaction-list?address=${address}&chainShortName=ETH_SEPOLIA&limit=50`);
      const res = await axios.get(
        `https://www.oklink.com/api/v5/explorer/address/token-transaction-list?address=${address}&chainShortName=ETH_SEPOLIA&limit=50`,
        { headers, timeout: 5000 }
      );
      const txList: any[] = res.data?.data?.[0]?.transactionLists || [];
  ...
  private async fetchWalletAge(address: string): Promise<number> {
    try {
      // Etherscan-compatible API — first transaction timestamp
      const headers = this.buildOkxHeaders("GET", `/api/v5/explorer/address/transaction-list?address=${address}&chainShortName=ETH_SEPOLIA&limit=1&page=999`);
      const res = await axios.get(
        `https://www.oklink.com/api/v5/explorer/address/transaction-list?address=${address}&chainShortName=ETH_SEPOLIA&limit=1&page=999`,
        { headers, timeout: 5000 }
      );
      const txList: any[] = res.data?.data?.[0]?.transactionLists || [];
