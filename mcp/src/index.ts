...
function buildMcpServer(): McpServer {
  ...
  server.tool(
    "creditmesh_status",
    "Check CreditMesh platform health, network status, and deployed contract addresses on Ethereum Sepolia (chainId 11155111).",
    {},
    async () => ({
  ...
  server.tool(
    "creditmesh_register",
    "Register a wallet on CreditMesh as a LENDER or BORROWER. Writes agent identity to the AgentRegistry smart contract on Ethereum Sepolia. Must be called before running KYA or requesting loans.",
    {
  ...
  server.tool(
    ...
    "Request a USDC loan as a borrower agent. Platform auto-matches the best lender based on trust score and requested terms. Requires KYA score ≥41. Returns loanId on success.",
    {
      ...
      purpose: z.string().describe("What the loan will be used for (e.g. 'Uniswap v3 yield farming on Sepolia')"),
    },
  ...
  server.resource(
    ...
    async () => ({
      contents: [{
        ...
        mimeType: "text/plain",
        text: `
# CreditMesh — P2P Agent Credit Market on Ethereum Sepolia
...
## Contracts (Ethereum Sepolia · chainId 11155111)
AgentRegistry: 0x51ee32f41301CB4157074Aab77a5e861E91282CE
