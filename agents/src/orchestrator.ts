...
import { saveAgentState, loadAgentState, logLoanEvent } from "./utils/ogStorage";
import { swapUsdcToWeth, swapWethToUsdc, getWethBalance } from "./utils/uniswap";
import { broadcastLoanRequest, isAXLAvailable, subscribe } from "./utils/axl";
...
// ── State Persistence (0G Storage KV + local file fallback) ───────────────────

async function saveState(s: AgentState[]) {
  const snapshot = s.map((st) => ({ id: st.record.id, activeLoanId: st.activeLoanId, cycleCount: st.cycleCount }));
  // Primary: 0G Storage KV
  await saveAgentState(snapshot);
  // Fallback: local file
  fs.writeFileSync(STATE_FILE, JSON.stringify(snapshot, null, 2));
}

async function loadSavedState(): Promise<Map<string, { activeLoanId: number | null; cycleCount: number }>> {
  // Try 0G Storage first
  const records: Array<{ id: string }> = fs.existsSync(STATE_FILE)
    ? JSON.parse(fs.readFileSync(STATE_FILE, "utf-8"))
    : [];
  const ids = records.map((r: any) => r.id);

  if (ids.length > 0) {
    const ogState = await loadAgentState(ids);
    if (ogState.size > 0) return ogState;
  }
...
async function handleLenderTool(name: string, input: any, state: AgentState): Promise<string> {
  ...
  switch (name) {
    ...
    case "top_up_deposit": {
      if (!record.wallet.privateKey) return JSON.stringify({ error: "No private key available" });
      try {
...
async function runLenderCycle(state: AgentState) {
  ...
  const system = `You are ${record.name}, an autonomous AI lending agent on CreditMesh (Ethereum Sepolia, USDC).
...
function borrowerTools(): Anthropic.Tool[] {
  return [
    ...
    {
      name: "swap_usdc_to_weth",
      description: "Swap borrowed USDC to WETH via Uniswap v3 on Sepolia to generate yield. Call after receiving a loan.",
      input_schema: {
        type: "object" as const,
        properties: {
          amount_usdc: { type: "string", description: "Amount of USDC to swap (e.g. '0.002')" },
        },
        required: ["amount_usdc"],
      },
    },
    {
      name: "swap_weth_to_usdc",
      description: "Swap WETH back to USDC via Uniswap v3 to close position before repaying loan.",
      input_schema: {
        type: "object" as const,
        properties: {
          amount_weth: { type: "string", description: "Amount of WETH to swap back (e.g. '0.000001')" },
        },
        required: ["amount_weth"],
      },
    },
    {
      name: "get_trust_score",
  ...
  switch (name) {
    ...
    case "request_loan": {
      ...
      if (result.loanId !== undefined) {
        ...
        await logLoanEvent({ type: "LOAN_CREATED", loanId: result.loanId, agent: record.wallet.address, amount: input.amount_usdc, ts: new Date().toISOString() });
      }
    ...
    case "repay_loan": {
      ...
      if (result.success) {
        await logLoanEvent({ type: "LOAN_REPAID", loanId: input.loan_id, agent: record.wallet.address, amount: "0", ts: new Date().toISOString() });
    ...
    case "swap_usdc_to_weth": {
      if (!record.wallet.privateKey) return JSON.stringify({ error: "No private key" });
      const signer = new ethers.Wallet(record.wallet.privateKey, provider);
      const result = await swapUsdcToWeth(signer, input.amount_usdc);
      return JSON.stringify(result);
    }
    case "swap_weth_to_usdc": {
      if (!record.wallet.privateKey) return JSON.stringify({ error: "No private key" });
      const signer = new ethers.Wallet(record.wallet.privateKey, provider);
      const result = await swapWethToUsdc(signer, input.amount_weth);
      return JSON.stringify(result);
    }
    case "get_wallet_balance": {
      const usdcToken  = new ethers.Contract(usdcAddr, ERC20_ABI, provider);
      const usdcBal    = await usdcToken.balanceOf(record.wallet.address);
      const wethBal    = await getWethBalance(provider, record.wallet.address);
      return JSON.stringify({
        balanceUsdc: ethers.formatUnits(usdcBal, USDC_DECIMALS),
        balanceWeth: wethBal,
      });
    }
    case "log": {
...
async function runBorrowerCycle(state: AgentState) {
  ...
  const system = `You are ${record.name}, an autonomous AI borrower agent on CreditMesh (Ethereum Sepolia, USDC).
...
Your job this cycle:
1. Check wallet USDC balance, WETH balance, and trust score
2. If you have an active loan:
   - If you hold WETH → call swap_weth_to_usdc first to close your Uniswap position
   - Then call repay_loan — fast repayment improves trust score
   - After repaying, request a new loan if balance allows
3. If no active loan:
   - Check available lenders
   - If a lender qualifies → request a loan (prefer durationHours: 2)
   - After receiving the loan → call swap_usdc_to_weth to deploy capital on Uniswap
4. Always explain your reasoning

Uniswap strategy: borrow USDC → swap to WETH (earn swap fees) → swap back → repay. Each round trip generates yield.`;
...
async function runCycle(states: AgentState[]) {
  ...
  console.log(`  Running ${states.length} agents on Ethereum Sepolia`);
...
async function main() {
  ...
  // Check AXL P2P mesh availability
  const axlUp = await isAXLAvailable();
  if (axlUp) {
    await subscribe("creditmesh.loan_request");
    await subscribe("creditmesh.loan_offer");
    console.log("🔗 Gensyn AXL mesh connected — P2P agent communication enabled\n");
  } else {
    console.log("⚠️  AXL node not found — using central API for agent coordination\n");
  }
