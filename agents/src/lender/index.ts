 ...
 * 1. Creates/loads a local EVM wallet on Ethereum Sepolia
 * 2. Registers on CreditMesh as LENDER
 * 3. Passes KYA (Trust Score computation)
 * 4. Deposits USDC liquidity and sets loan terms
 * 5. Monitors active loans and re-lends earnings (economy loop)
 *
 * Brain: Claude API (claude-haiku-4-5-20251001) with tool use
 */
...
const tools: Anthropic.Tool[] = [
  {
    name: "create_wallet",
    description: "Create a new EVM wallet for this lender agent.",
    input_schema: {
  ...
  },
  {
    ...
    input_schema: {
      type: "object" as const,
      properties: {
        wallet_address: { type: "string", description: "The EVM wallet address" },
      },
  ...
  },
  {
    ...
    description: "Get the ETH balance of a wallet on Ethereum Sepolia.",
    input_schema: {
      type: "object" as const,
      properties: {
        wallet_address: { type: "string", description: "Wallet address" },
  ...
  },
  {
    ...
    description: "Deposit USDC liquidity into the LoanEscrow contract.",
    input_schema: {
      type: "object" as const,
      properties: {
        wallet_address: { type: "string", description: "Lender wallet address" },
        amount_eth: { type: "string", description: "Amount of USDC to deposit" },
        ...
        max_loan_size_eth: { type: "string", description: "Maximum loan size in USDC" },
...
async function runLenderAgent() {
  ...
  console.log("║     Autonomous Lending on Ethereum Sepolia       ║");
  console.log("╚═══════════════════════════════════════╝\n");

  const systemPrompt = `You are an autonomous AI LenderAgent operating on CreditMesh, a decentralized lending platform on Ethereum Sepolia blockchain.

Your mission:
1. Create an EVM wallet as your onchain identity
...
Deposit USDC from your funded wallet to start earning yield.
