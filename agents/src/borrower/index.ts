 ...
 * 1. Creates/loads an EVM wallet on Ethereum Sepolia
 ...
 * 5. Deploys borrowed USDC via Uniswap v3 to earn yield
 * 6. Repays the loan + interest before deadline
 *
...
const tools: Anthropic.Tool[] = [
  {
    name: "create_wallet",
    description: "Create a new EVM wallet on Ethereum Sepolia for this borrower agent.",
    input_schema: {
  ...
  {
    name: "repay_loan",
    description: "Repay a loan. Sends principal + interest back onchain.",
    input_schema: {
      type: "object" as const,
      properties: {
        loan_id: { type: "number" },
...
async function handleTool(name: string, input: any): Promise<string> {
  try {
    switch (name) {
      ...
      case "repay_loan": {
        // Confirm repayment onchain via LoanEscrow.recordRepayment()
...
async function runBorrowerAgent() {
  ...
  console.log("║   Autonomous Borrowing on Ethereum Sepolia  ║");
  console.log("╚═══════════════════════════════════════╝\n");

  const systemPrompt = `You are an autonomous AI BorrowerAgent operating on CreditMesh, a P2P credit market for AI agents on Ethereum Sepolia.
